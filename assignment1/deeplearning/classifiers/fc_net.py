...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    dout, dw2, db2 = affine_backward(dout, af_cache)
    dout, dw1, db1 = affine_relu_backward(dout, arf_cache)
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for lay in xrange(self.num_layers-1):
        if self.use_batchnorm:
            layer_input, ar_cache[lay] = affine_bn_relu_forward(layer_input,
                                        self.params['W' + str(lay+1)], self.params['b' + str(lay+1)],
                                        self.params['gamma' + str(lay+1)], self.params['beta' + str(lay+1)], self.bn_params[lay])
        else:
            layer_input, ar_cache[lay] = affine_relu_forward(layer_input, self.params['W' + str(lay+1)], self.params['b' + str(lay+1)])

        if self.use_dropout:
            layer_input,  dp_cache[lay] = dropout_forward(layer_input, self.dropout_param)

    ar_out, ar_cache[self.num_layers] = affine_forward(layer_input, self.params['W' + str(self.num_layers)], self.params['b' + str(self.num_layers)])
    scores = ar_out
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dhout = dscores
    loss = loss + 0.5 * self.reg * np.sum(self.params['W' + str(self.num_layers)] * self.params['W' + str(self.num_layers)])
    ...
    for idx in xrange(self.num_layers-1):
        ...
        loss = loss + 0.5 * self.reg * np.sum(self.params['W' + str(lay+1)] * self.params['W' + str(lay+1)])
        if self.use_dropout:
...
# function added in part 2: Batch Normalization
def affine_bn_relu_forward(x, w, b, gamma, beta, bn_param):
    """
    Convenience layer that performs an affine, a batch normalization and a ReLU.

    Inputs:
    - x: Input to the affine layer
    - w, b: Weights and bias for the affine layer
    - gamma, beta, bn_param: Scale and shift parameters for batch normalization layer
